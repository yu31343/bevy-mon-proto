...
>
> UI text depends on the build script finding at least one `.ttf`, `.otf`, or `.ttc` file under `assets/fonts/`. Builds fail fast if that directory has no usable font, and the build log prints which font file was embedded.
>
> Gameplay data is loaded from `assets/data/battle_data.ron` via a fixed relative path, so run the game from the repository root.

## Architecture

`src/main.rs` is intentionally thin: it initializes `GameState` and `BattlePhase`, then registers `DataPlugin`, `UiPlugin`, `TeamSelectionPlugin`, `BattlePlugin`, and `SpineAnimPlugin` in that order.
...
`R` on the result screen clears the previous selection and returns to `GameState::TeamSelection`.

### Data and battle setup

- `src/data/mod.rs` loads and validates `assets/data/battle_data.ron` at startup.
- That RON file is the gameplay source of truth: battle rules, element matrix, skills, monster prototypes, card definitions, and deck composition all come from it.
- Startup inserts `BattleDbs`, `MonsterPool`, `CardDeck`, `BattleRules`, and `BattleDataStatus`. `BattleDbs` intentionally bundles skills, cards, and elements into one resource to stay under Bevy's system-parameter limit.
- If loading or validation fails, startup still inserts fallback resources plus a populated `BattleDataStatus.error`; systems should treat that resource as the authoritative failure signal.
...
- `init_battle_system` waits for `TeamSelections`, despawns all `InBattle` entities from any prior run, resets per-battle resources, validates the chosen indices, and spawns the selected monsters into `PlayerTeam` and `EnemyTeam`.
...
- `end.rs`: fainting, death-resolution, auto-switching, and result transitions.
...
- `PendingBoosts` stores card-granted bonuses for the next matching attack, heal, or shield action and is consumed on use.
- `check_end_system` does not immediately declare defeat when the active combatant faints; it first transitions through `BattlePhase::DeathResolve`, then auto-switches or moves to `GameState::Result` if a side has no living members left.
...
- `BattlePlugin` uses Bevy messages (`add_message::<BattleEvent>()`) as the contract between battle logic and downstream presentation systems.
...
- Core per-battle flow is carried by resources such as `PlayerTeam`, `EnemyTeam`, `TurnContext`, `ActionPoints`, `Hand`, `PendingBoosts`, `SelectedCard`, `BattleLog`, `BattleResult`, and `PendingKoResolution`.
...
- `src/ui/mod.rs` owns battle UI lifecycle details that are easy to miss: it spawns the camera and loads the embedded font at `Startup`, creates battle UI on `OnEnter(GameState::Battle)`, and cleans it up on `OnEnter(GameState::TeamSelection)`.
- Team selection UI is separate from battle UI and lives under `src/team_selection/`. Its setup runs in `Update` with resource/state guards, not a simple `OnEnter`, so missing prerequisites can prevent the screen from appearing.
- `src/ui/battle/fx.rs` owns timed visual feedback such as flashes and other short-lived battle effects.
- `src/spine_anim.rs` registers `SpineAnimPlugin`, which loads skeleton assets at startup and reacts to battle messages during battle/result flow. Presentation changes may need matching updates in both UI and spine animation systems.
